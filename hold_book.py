...
class HoldBook:
    def __init__(self, root):
        ...
        self.root.title("Search Book to hold")
        
        self.label = Label(self.root, text="Search Book to hold", fg="#1c6f9c", font=("Times New Roman", 20, "bold"))
        self.label.pack()
    ...
    def select_book(self):
        ...
        if selected_item:
            ...
            print("Please select a book to hold.")
    def open_update_window(self, book_id):
        update_window = Toplevel(self.root)
        update_window.title("Hold Book")
        ...
        Label(update_window, text="Hold Quantity").grid(row=2, column=0, padx=10, pady=10)
        ...
        qty_bal = StringVar(value="3")
        bal = tk.Entry(update_window, textvariable=qty_bal, width=40)
        bal.grid(row=2, column=1, padx=10, pady=10)
        bal.bind("<FocusOut>", self.on_focus_out)
        ...
        borrow_button = Button(update_window, image=self.btnimg, bd=0, cursor="hand2",
                               command=lambda: self.hold_book(book_id, title_var, bal, member, rollno, update_window))
        borrow_button.grid(row=5, column=0, columnspan=2, pady=10)

    def hold_book(self, book_id, title_var, bal, member, rollno, update_window):
        id = book_id
        title = title_var.get()
        balance = bal.get()
        mbr = member.get()
        ...
        if result1 :
            q2 = "SELECT SUM(holdqty) FROM holds WHERE member_name=%s AND member_roll=%s"
            vals2 = (mbr, roll, )
            ...
            if result2 >= 3 or result2+qty > 3 :
                messagebox.showwarning("Limit Exceeded", f"Only maximum 3 books can hold.")
            else :
                query = "INSERT INTO holds (book_id, title, holdqty, member_name, member_roll, hold_date) VALUES (%s,%s,%s,%s,%s,CURDATE())"
                values = (id, title, qty, mbr, roll, )
                mycursor = dbcon.cursor()
                mycursor.execute(query, values)
                dbcon.commit()
                messagebox.showinfo("Hold Success", f"Successfully holded!")
    ...
    def on_focus_out(self, event):
        ...
        if current_text > max_chars :
            messagebox.showwarning("Limit Exceeded", f"Only maximum {max_chars} books can hold.")
